...
import { initSecurity } from './services/securityLayer';
...
const TurntableChat: React.FC<{ language: Language; t: (k: string) => string }> = ({ language, t }) => {
  ...
  // Improved Voice Input Logic with Rapid Interim Transcription
  const handleVoiceInput = () => {
    ...
    recognition.continuous = false; // Capture one sentence/phrase at a time for better accuracy
    recognition.interimResults = true; // Key for rapid transcription
    
    // Set language dynamically based on app selection to handle accents correctly
    switch (language) {
      case 'fa':
    ...
    recognition.onstart = () => {
      setIsListening(true);
      // Optional: Visual cue for start (handled by isListening state)
    };
    ...
    recognition.onresult = (event: any) => {
      ...
      // If we have a final result, append it. If we have interim, show it (overwrite input temporarily).
      // Note: This logic assumes input is cleared before speaking or user is appending.
      // For simplicity in this chat interface, we replace input with what's being said if it's new.
    ...
    recognition.onend = () => {
      setIsListening(false);
      // Auto-send could be implemented here if desired, but user might want to edit.
    };
